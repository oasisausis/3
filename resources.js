(() => {
  ...
  function card(resource, index) {
    return `<article class="resource-card resource-${escapeHtml(resource.accent)}" data-category="${escapeHtml(resource.category)}" data-search="${escapeHtml((resource.title+' '+resource.category+' '+resource.description).toLowerCase())}">
      <button class="resource-preview-button" type="button" data-resource-index="${index}" aria-label="Preview ${escapeHtml(resource.title)}">
        <img src="${escapeHtml(resource.thumbnail)}" alt="First page of ${escapeHtml(resource.title)}" loading="lazy">
      ...
      <div class="resource-card-body">
        ...
        <div class="resource-card-actions">
          <button type="button" class="btn btn-primary resource-inline-preview" data-resource-index="${index}">Preview</button>
          <a class="btn btn-secondary" href="${escapeHtml(resource.url)}" target="_blank" rel="noopener">Open PDF ↗</a>
        </div>
      </div>
    </article>`;
  }
  ...
  function select(index) {
    ...
    frame.src = `${resource.url}#view=FitH`;
